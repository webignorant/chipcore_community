...
if (mysql_query("use chipcore_community;", $link)) {
    echo "选择chipcore_community数据库成功" . "<br>";
} else {
    echo "选择chipcore_community数据库失败" . mysql_error() . "<br>";
    exit;
}

//显示所有数据库名称
$result = mysql_query("SHOW TABLES FROM chipcore_community", $link);
echo "已存在数据库列表：";
echo "<hr>";
...
if (mysql_query("drop table if exists common_application;")) {
    echo "清除common_application表成功" . "<br>";
} else {
    echo "清除common_application表失败" . mysql_error() . "<br>";
    exit;
}
if (mysql_query("
