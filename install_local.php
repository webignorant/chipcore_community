...
$link=mysql_connect("127.0.0.1","root","") or die("数据库连接失败".mysql_error());
mysql_query("set names utf8");

if(mysql_query("
CREATE DATABASE if not exists `chipcorecommunity` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
",$link))
{
	echo "创建数据库成功！"."<br>";
}else
...
if(mysql_query("use chipcorecommunity;",$link))
{
	echo "选择chipcorecommunity数据库成功"."<br>";
}else
{
	echo "选择chipcorecommunity数据库失败".mysql_error()."<br>";
	exit;
}

//显示所有数据库名称
$result = mysql_query("SHOW TABLES FROM chipcorecommunity",$link);
echo "已存在数据库列表：";
echo "<hr>";
